...
async function getPosts(page: number, q?: string, categorySlug?: string) {
  ...
  const where: Prisma.PostWhereInput = { published: true, showForGen: true };
  if (q?.trim()) {
    const k = q.trim();
    where.OR = [
...
async function getPickupPosts(): Promise<Post[]> {
  try {
    ...
    const pickups = await prisma.post.findMany({
      where: { published: true, isPickup: true, showForGen: true } as Prisma.PostWhereInput,
...
async function getBanners(): Promise<Banner[]> {
  return prisma.banner.findMany({
    where: { OR: [{ media: "gen" }, { media: "all" }] },
    orderBy: { order: "asc" },
    select: { id: true, label: true, url: true, imageUrl: true, media: true, order: true },
  });
}

export default async function GenMemberPage({
  searchParams,
}: {
  ...
  // ページネーション用basePath
  const paginationBase = catSlug ? `/gen?cat=${catSlug}` : "/gen";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
            ...
            <div className="pt-4 pb-2 border-b border-black/10 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              <div className="flex items-center gap-1 min-w-max">
                ...
                {menuCategories.map((cat) => (
                  <Link key={cat.id} href={`/gen?cat=${cat.slug}`}
                    className={`px-4 py-2 rounded-full text-sm font-semibold transition-colors whitespace-nowrap ${
          ...
          <div className="lg:flex lg:gap-10">
            <div className="flex-1 min-w-0">
                ...
                <section className="pt-8 pb-6 border-b border-black/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {pickupPosts.map((post: Post) => (
                      <PostCard key={post.id} post={post} variant="pickup" basePath="/gen" />
                    ))}
                  </div>
                </section>
              )}
              <section className="pt-8 pb-6">
                ...
                {data.posts.length > 0 ? (
                  <>
                    <div className="hidden md:block">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="list" basePath="/gen" />
                      ))}
                    </div>
                    <div className="md:hidden grid grid-cols-2 gap-x-4 gap-y-6 pt-6">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="grid" basePath="/gen" />
                      ))}
                    </div>
                  ...
                  <div className="text-center py-20">
                    ...
                    <p className="text-black/40 text-sm mt-1">{q ? "検索条件に一致する記事がありません" : activeCategoryName ? `${activeCategoryName}の記事はありません` : "一般会員向けの記事はありません"}</p>
                  </div>
                )}
              </section>
              ...
              <div className="lg:hidden">
                ...
                <div className="pt-6 pb-4 border-t border-black/10">
                  <form action="/gen" method="get" className="relative max-w-2xl">
                    ...
                    <ul className="space-y-4">
                      {recommended.map((post) => (
                        <li key={post.id} className="border-b border-black/5 last:border-0 pb-4 last:pb-0">
                          <Link href={`/gen/${post.slug}`} className="group flex gap-3">
            ...
            <aside className="hidden lg:block w-[320px] shrink-0 pt-10">
              <div className="mb-6">
                <form action="/gen" method="get" className="relative">
                  <input type="hidden" name="page" value="1" />
