...
async function getRecommended(): Promise<Post[]> {
  try {
    return await prisma.post.findMany({
      where: { published: true, showForVC: true } as Prisma.PostWhereInput,
      orderBy: { views: "desc" },
      take: 5,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="lg:flex lg:gap-10">
                ...
                <section className="pt-8 pb-6 border-b border-black/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {pickupPosts.map((post: Post) => (
                      <PostCard key={post.id} post={post} variant="pickup" basePath="/vc" />
                    ))}
                  </div>
                </section>
              )}
                  ...
                  <>
                    <div className="hidden md:block">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="list" basePath="/vc" />
                      ))}
                    </div>
                    ...
                    <ul className="space-y-4">
                      {recommended.map((post) => (
                        <li key={post.id} className="border-b border-black/5 last:border-0 pb-4 last:pb-0">
                          <Link href={`/vc/${post.slug}`} className="group flex gap-3">
            ...
            <aside className="hidden lg:block w-[320px] shrink-0 pt-10">
              <div className="mb-6">
                <form action="/vc" method="get" className="relative">
                  <input type="hidden" name="page" value="1" />
